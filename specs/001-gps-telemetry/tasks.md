...
- [ ] T001 Create `requirements.txt` with the `pyserial` runtime dependency per Constitution I.
- [ ] T002 Create the `unittest` discovery command and test-file layout in `test_parser.py`,
  `test_client.py`, and `test_integration.py` per FR-013.
- [ ] T003 [P] Document the physical deployment verification for dual memory capacity and connector
...
- [ ] T004 Write failing command-frame tests in `test_parser.py` for `0x7E | CMD_ID | LEN | DATA |
  XOR`, length mismatch, invalid XOR, and unknown command rejection per FR-007 and FR-016.
- [ ] T005 Write failing state and recovery tests in `test_client.py` for `INIT` to `STANDBY`,
  `SCIENCE_START` (`0x01`), `SyncError`, and the ordered recovery actions per FR-004 through
  FR-006.
- [ ] T006 Implement command-frame encoding and validation in `gps_client.py`, including `0x01`
  (`SCIENCE_START`), `0x02` (`SU_R_SDP`), and `0x03` (`SU_R_HK`) per FR-007 and FR-016.
- [ ] T007 Implement the explicit operational-state model and legal transitions in `gps_client.py`
  per FR-005.
- [ ] T008 Implement `SyncError` handling and the ordered abort, `SU_R_SDP`, `SU_R_HK`,
...
- [ ] T012 [US2] Write failing serial-configuration and timeout tests in `test_client.py` for 9600
  8N1 and the 500 ms `SyncError` deadline per FR-002 and FR-004.
- [ ] T013 [US2] Implement serial-port initialization with 9600 8N1 and a 500 ms timeout in
  `gps_client.py` per FR-002 and FR-004.
- [ ] T014 [US2] Implement the client control loop that processes commands, enters `SCIENCE` only
...
- [ ] T015 [US3] Write failing NMEA parser tests in `test_parser.py` for `$GPGGA` field extraction,
...
- [ ] T021 [US4] Complete end-to-end client/simulator and PTY-loss verification in
